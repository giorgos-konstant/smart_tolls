...
// TOLL-PAYMENT
messageEmitter.on('tollMessage', async ({toll_name, device_id, timestamp})=> {
    try {
        ...
        const toll = await TollModel.findOne({region: toll_name}).exec();
        if(!toll) {
        ...
        if (!policy) {
            return res.status(404).send({ message: 'Charging policy not found' });
          }
